...
if rank==main:
    ...
    def cost(coeff_l):
        ...
        err_tot=avg_surf_err+lamb*avg_err_j+gamma*avg_Laplace
        logging.info('cost : surface error = {:.5E} ,\n j norm = {:.5E} \n Laplace norm = {:.5E}\n total cost = {:.5E}'.format(avg_surf_err,avg_err_j,avg_Laplace,err_tot))
...
if rank==main:
    def grad(coeff_l):
        #we send the message to continue
        ...
        return mc*(dcdreg+gamma*dcLdc)
    #print(grad(coeff_l).shape)
    ...
    while bool_continue:
        ...
        if np.equal(-1*np.ones(lc),coeff_l).all():
            ...
            partial_grad=avg.grad_f_laplace(lu-1,lv-1,coeff,djdc,blocks,lst_zeta)
            comm.send(partial_grad, dest = main)
            flag+=1
            if rank==0:
                logging.info('proc {} done task {} in {}'.format(rank,flag,time.time()-t1))

    logging.info('proc {} done'.format(rank))
